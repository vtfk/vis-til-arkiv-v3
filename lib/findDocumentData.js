...
module.exports = {
  ...
  visVarselDoc: (documentType, pdfStrings) => {
    ...
    documentData.birthdate = `${birthdateList[0]}${birthdateList[1]}${birthdateList[2].substring(0,2)}`
  ...
  visKarakterutskrift: (documentType, pdfStrings) => {
    // Check if this is a pdf with more than one document
    const pdfSentences = []
    for (const item of pdfStrings) {
      pdfSentences.push(...item.split(' ').filter(word => /\S/.test(word)))
    ...
    for (const str of pdfStrings) {
      // Get school
      try {
        const schoolInf = getSchoolInfo(str, documentType)
