...
module.exports = {
  moveToNextJob: (jsonRes, jsonFile, jobDir, nextJob) => {
    ...
    logger('info', ['Vis-til-Arkiv', `Job "${jobDir.substring(jobDir.lastIndexOf('/') + 1, jobDir.length)}" succeded :), moved files to next job: "${jobDir}/${nextJob}"`, jsonRes.pdf])
  },
  handleError: async (jsonRes, jsonFile, jobDir, msg, error, retry) => {
    if ((jsonRes.retries >= retryCount) || !retry) {
      ...
      if (retry) {
        logger('error', ['Vis-til-Arkiv', `Failed in job ${jobDir.substring(jobDir.lastIndexOf('/') + 1, jobDir.length)}. Have retried ${retryCount} times now, won't do it again... MESSAGE: ${msg}`, `FILE: ${jobDir}/${jsonRes.pdf}`, `ERROR: ${error}`])
        await teamsError(`Failed in job ${jobDir.substring(jobDir.lastIndexOf('/') + 1, jobDir.length)}. Have retried ${retryCount} times now, won't do it again... MESSAGE: ${msg}`, `FILE: ${jobDir}/${jsonRes.pdf}`, `ERROR ${error}`)
      } else {
        logger('error', ['Vis-til-Arkiv', `Failed in job ${jobDir.substring(jobDir.lastIndexOf('/') + 1, jobDir.length)}. Retry is off for this task. MESSAGE: ${msg}`, `FILE: ${jobDir}/${jsonRes.pdf}`, `ERROR: ${error}`])
        await teamsError(`Failed in job ${jobDir.substring(jobDir.lastIndexOf('/') + 1, jobDir.length)}. Have retried ${retryCount} Retry is off for this task. MESSAGE: ${msg}`, `FILE: ${jobDir}/${jsonRes.pdf}`, `ERROR ${error}`)
      }
    } else {
      ...
      logger('error', ['Vis-til-Arkiv', `Failed in job ${jobDir.substring(jobDir.lastIndexOf('/') + 1, jobDir.length)}. Have retried ${retries} times now, will try again in ${retryTime[retries]} minutes. MESSAGE: ${msg}`, `FILE: ${jobDir}/${jsonRes.pdf}`, `ERROR: ${error}`])
      await teamsError(`Failed in job ${jobDir.substring(jobDir.lastIndexOf('/') + 1, jobDir.length)}. Have retried ${retries} times now, will try again in ${retryTime[retries]} minutes. MESSAGE: ${msg}`, `FILE: ${jobDir}/${jsonRes.pdf}`, `ERROR ${error}`)
    }
  },
  shouldRun: (nextTry) => {
