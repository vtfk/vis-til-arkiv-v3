...
module.exports = {
  ...
  handleError: async (jsonRes, jsonFile, jobDir, msg, error, retry) => {
    if ((jsonRes.retries >= retryCount) || !retry) {
      ...
      saveJsonDocument(`${jobDir}/error/${path.basename(jsonRes.pdf).substring(0, path.basename(jsonRes.pdf).lastIndexOf('.'))}.json`, { ...jsonRes, lastError: error.toString() })
      if (retry) {
        logger('error', ['Vis-til-Arkiv', `Failed in job ${jobDir.substring(jobDir.lastIndexOf('/') + 1, jobDir.length)}. Have retried ${retryCount} times now, won't do it again... MESSAGE: ${msg}`, `FILE: ${jsonRes.pdf}`, `ERROR: ${error.toString()}`])
        ...
        logger('error', ['Vis-til-Arkiv', `Failed in job ${jobDir.substring(jobDir.lastIndexOf('/') + 1, jobDir.length)}. Retry is off for this task. MESSAGE: ${msg}`, `FILE: ${jsonRes.pdf}`, `ERROR: ${error.toString()}`])
      ...
      saveJsonDocument(`${jobDir}/${path.basename(jsonRes.pdf).substring(0, path.basename(jsonRes.pdf).lastIndexOf('.'))}.json`, { ...jsonRes, retries: retries + 1, nextTry: nextTry.toISOString(), lastError: error.toString() })
      logger('error', ['Vis-til-Arkiv', `Failed in job ${jobDir.substring(jobDir.lastIndexOf('/') + 1, jobDir.length)}. Have retried ${retries} times now, will try again in ${retryTime[retries]} minutes. MESSAGE: ${msg}`, `FILE: ${jsonRes.pdf}`, `ERROR: ${error.toString()}`])
